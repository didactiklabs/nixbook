...
semver_ge() {
    ...
    split_semver "$1"; a_maj=$MAJ; a_min=$MIN; a_pat=$PAT
    split_semver "$2"; b_maj=$MAJ; b_min=$MIN; b_pat=$PAT
...
engine_satisfies() {
    ...
    if [[ "$constraint" == "^"* ]]; then
        ...
        split_semver "$min_ver"; local c_maj=$MAJ
        split_semver "$vscode_ver"; local v_maj=$MAJ
