...
let
  ...
  isInNixpkgs =
    ext:
    (
      if builtins.hasAttr (lib.toLower ext.publisher) pkgs.vscode-extensions then
        builtins.hasAttr (lib.toLower ext.name) pkgs.vscode-extensions."${lib.toLower ext.publisher}"
      else
        false
    );
  ...
  extensionsNotInNixpkgs = pkgs.vscode-utils.extensionsFromVscodeMarketplace (
    builtins.filter (n: !(isInNixpkgs n)) extensions
  );

  ## Filter extensionsList.nix
  ## Returns a list with the derivation for the extensions that are in nixpkgs
  extensionsInNixpkgs = map (
    v: pkgs.vscode-extensions."${lib.toLower v.publisher}"."${lib.toLower v.name}"
