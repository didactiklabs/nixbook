    ...
    programs.waybar = {
      ...
      settings = [{
        ...
        "pulseaudio" = {
          ...
          on-click =
            "sleep 0.1 && ${pkgs.wireplumber}/bin/wpctl set-mute @DEFAULT_SINK@ toggle";
          on-click-middle = "${pkgs.pavucontrol}/bin/pavucontrol";
        };
        "custom/exit" = {
          tooltip = false;
          format = "";
          on-click = lib.mkIf cfg.rofiConfig.enable
            "sleep 0.1 && $HOME/.config/rofiScripts/rofiLockScript.sh ${rofiPowermenuStyle}";
        };
        "custom/startmenu" = {
          tooltip = false;
          format = "";
          # exec = "rofi -show drun";
          on-click = lib.mkIf cfg.rofiConfig.enable
            "sleep 0.1 && ${rofi-wayland} -show drun -theme $HOME/.config/rofi/launchers/${rofiLauncherType}/${rofiLauncherStyle}.rasi";
        };
        "custom/hyprbindings" = {
          tooltip = false;
          format = "󱕴";
        ...
        "custom/spotify" = {
          ...
          tooltip = false;
          on-click =
            "${pkgs.playerctl}/bin/playerctl --player=spotify previous";
          on-click-middle =
            "${pkgs.playerctl}/bin/playerctl --player=spotify play-pause";
          on-click-right =
            "${pkgs.playerctl}/bin/playerctl --player=spotify next";
        };
        "custom/notification" = {
          tooltip = false;
          format = "{icon} {}";
          ...
          on-click =
            "sleep 0.1 && ${pkgs.swaynotificationcenter}/bin/swaync-client -t &";
          escape = true;
        };
        "battery" = {
          states = {
