...
| -------------------- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `core.nix`           | 396   | Foundational system: systemd-boot UEFI, plymouth, latest kernel, LVM, LUKS, PipeWire audio, kernel sysctl hardening, security (rtkit, polkit, U2F PAM), GNOME Keyring system support (`services.gnome.gnome-keyring`: Secret Service D-Bus activation, gcr unlock prompter, Secret portal backend, PAM login unlock, cap_ipc_lock wrapper), Lix nix daemon with S3 cache, fprintd, chrony, fwupd, networkmanager |
| `userConfig.nix`     | 302   | User management framework with mkUser helper, Home Manager integration, per-user gnome-keyring daemon (`services.gnome-keyring`, components pkcs11+secrets only — ssh stays with gpg-agent), Qt theming (qt5ct/qt6ct installed system-wide with their launcher .desktop entries hidden via a NoDisplay=true postInstall override), Go dev environment                                                            |
...
| `niri.nix`           | 127   | Niri scrollable tiling compositor: fuzzel, grimblast, wl-clipboard, xwayland-satellite, Secret portal → gnome-keyring, polkit-gnome agent, hyprlock PAM service (U2F + keyring re-unlock for the HM fallback locker)                                                                                                                                                                                             |
...
- `foxblatConfig.nix` (398 LOC) - Foxblat (Moza Racing) declarative FFB presets (ACC, AC1, Cyberpunk 2077) in ~/.config/foxblat/presets/, autostart, settings stamping. Requires `customNixOSModules.simracing`. Used on anya
- `oversteerConfig.nix` (~140 LOC) - Oversteer (Fanatec) wheel profile in ~/.config/oversteer/profiles/acc.ini for the CSL DD / GT DD Pro + CSL Pedals (900° range, autocenter off for direct drive). Applies on login via XDG autostart. Cannot set the Fanatec base tune (FFB/NDP/NFR/NIN/FEI) — that lives on the wheelbase OLED menu. Requires `customNixOSModules.simracing`. Used on hanamichi
