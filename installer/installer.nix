  ...
  config.system.build.scripts = {
    installer = pkgs.writeScriptBin "installer" ''
            ...
            wipefs -af "$TARGET_DISK" || true
             ...
             # Create a minimal hardware-configuration.nix
             # It will be regenerated by the bootstrap process after the first boot
             echo "{ ... }:
      {
        # This is a minimal hardware configuration that will be replaced
        # by the bootstrap process after the first boot.
        # The actual device detection will happen after the system boots.
